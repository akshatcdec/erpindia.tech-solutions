    ...
    public class TeacherAssignmentController : BaseController
    {
        ...
        public JsonResult GetExistingAssignments()
        {
            try
            {
                ...
                using (var connection = new SqlConnection(connectionString))
                {
                    // Query to get all teacher subject assignments for validation
        ...
        [HttpGet]
        public JsonResult GetTeacherAssignmentList(int clientId)
        {
            try
            {
                ...
                // Build query to get teacher assignments with names
                string query = @"
                SELECT tsa.Id, tsa.TeacherID, tsa.ClassID, tsa.SectionID, tsa.SubjectID,
                ...
                INNER JOIN Configurations t ON tsa.TeacherID = t.Id AND t.KeyName = 'TEACHER'
                INNER JOIN Configurations c ON tsa.ClassID = c.Id AND c.KeyName = 'CLASS'
                INNER JOIN Configurations s ON tsa.SectionID = s.Id AND s.KeyName = 'SECTION'
                INNER JOIN Configurations sub ON tsa.SubjectID = sub.Id AND sub.KeyName = 'SUBJECT'
                WHERE tsa.ClientID = @ClientID
        ...
        [HttpPost]
        public JsonResult SaveTeacherAssignment(TeacherAssignmentModel model)
        {
            try
            {
                int clientId = GetCurrentClientId();
                ...
                using (var connection = new SqlConnection(connectionString))
                {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            {
                                string deleteQuery = @"DELETE FROM TeacherSubjectAssignments 
                                                     WHERE ClientID = @ClientID
                                ...
                                foreach (var cls in model.classes)
                                {
                                    foreach (var section in model.sections)
                                    {
                                        foreach (var subject in model.subjects)
                                        {
                                            ...
                                            string checkQuery = @"SELECT COUNT(1) FROM TeacherSubjectAssignments 
                                                               WHERE ClientID = @ClientID 
                                            ...
                                            if (existingCount == 0 || model.forceOverride)
                                            {
                                                ...
                                                string insertSql = @"INSERT INTO TeacherSubjectAssignments 
        ...
        [HttpPost]
        public JsonResult DeleteTeacherAssignment(int id, int clientId)
        {
            try
            {
                ...
                using (var connection = new SqlConnection(connectionString))
                {
                    ...
                    string verifyQuery = @"SELECT COUNT(1) FROM TeacherSubjectAssignments 
                    ...
                    string deleteQuery = "DELETE FROM TeacherSubjectAssignments WHERE Id = @Id AND ClientID = @ClientID";
